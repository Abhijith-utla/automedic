...
const SIDE_EFFECT_CHIP: Record<string, string> = {
  ...
  hypotension: "bg-violet-50 text-violet-700 border-violet-200",
  bradycardia: "bg-sky-50 text-sky-700 border-sky-200",
  nausea: "bg-orange-50 text-orange-700 border-orange-200",
  default: "bg-slate-50 text-slate-600 border-slate-200",
};

const ICD_CATEGORY_STYLE: Record<string, { badge: string; bar: string }> = {
  Symptom: { badge: "bg-amber-50 text-amber-700 border-amber-200", bar: "bg-amber-400" },
  Psych: { badge: "bg-violet-50 text-violet-700 border-violet-200", bar: "bg-violet-400" },
  Musculoskeletal: { badge: "bg-sky-50 text-sky-700 border-sky-200", bar: "bg-sky-400" },
...
export function MedicationsICDTab({ data }: { data: ChestPainLabeledReport }) {
  ...
  // Gather all symptoms for bipartite mapping
                ...
                {medications.map((med) => {
                  ...
                  return (
                    <>
                      ...
                      {expanded && (
                        <tr key={`${med.name}-expanded`} className="bg-slate-50">
      ...
      {medications.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex gap-3 mt-3 text-[10px]">
            ...
            <span className="flex items-center gap-1"><span className="h-2 w-5 rounded bg-amber-50 border border-amber-200" /> Medium</span>
          ...
          <div className="flex gap-8 items-start min-w-[480px]">
            ...
            <div className="flex flex-col gap-3 shrink-0">
              ...
              {symptoms.map((sym, i) => (
                <div
                  key={i}
                  className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-medium text-sky-800 whitespace-nowrap"
                >
                  {sym}
                </div>
              ))}
